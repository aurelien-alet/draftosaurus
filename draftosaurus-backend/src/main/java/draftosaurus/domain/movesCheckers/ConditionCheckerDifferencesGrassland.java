...
public class ConditionCheckerDifferencesGrassland extends ConditionChecker {
    
    private boolean ANY_DINOSAURE_CAN_BE_ASSIGNED = true;
    private boolean DINOSAURE_DIFFERENT_SPECY_TO_ASSIGNED_CAN_BE_ASSIGNED = true;
    private boolean DINOSAURE_SIMILAR_SPECY_TO_ASSIGNED_CANNOT_BE_ASSIGNED = false;

    public ConditionCheckerDifferencesGrassland(PaddockAssignements paddockAssignements) {
    ...
    private boolean isDinosaureAssignableToAlreadyAssignedPaddock(Dinosaure dinosaure){
        ...
        if( paddockAssignements.numberOfAssignedSpecy(dinosaureToAssignSpecy) > 0 ){
            return  DINOSAURE_SIMILAR_SPECY_TO_ASSIGNED_CANNOT_BE_ASSIGNED;
        } else {
            return DINOSAURE_DIFFERENT_SPECY_TO_ASSIGNED_CAN_BE_ASSIGNED;
        }
    }
